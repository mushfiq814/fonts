...
def get_ligature_source(fontname):
    ...
    for weight in ['Bold', 'Retina', 'Medium', 'Regular', 'Light']:
        if fontname.endswith('-' + weight.lower()):
            ...
            return 'fonts/fira/FiraCode-%s.ttf' % weight
    ...
    if 'bold' in fontname or 'heavy' in fontname:
        return 'fonts/fira/FiraCode-Bold.ttf'
    return 'fonts/fira/FiraCode-Regular.ttf'
...
class LigatureCreator(object):
    ...
    def add_calt(self, calt_name, subtable_name, spec, **kwargs):
        ...
        self.font.addContextualSubtable(
            calt_name, subtable_name, 'glyph', spec)
...
def ligaturize_font(input_font_file, output_dir, ligature_font_file,
    ...
    for lig_spec in sorted(ligatures, key=ligature_length):
        try:
            creator.add_ligature(
                lig_spec['chars'], lig_spec['firacode_ligature_name'])
    ...
    output_font_file = path.join(output_dir, name + output_font_type)
    print("    ...saving to '%s' (%s)" % (output_font_file, name))
    flags = (
        str("opentype"),
        str("no-FFTM-table"),
    )
    font.generate(output_font_file, flags=flags)
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        help="The directory to save the ligaturized font in. The actual"
        " filename will be automatically generated based on the input font"
        " name and the --prefix and --output-name flags."
    )
    parser.add_argument(
        "--ligature-font-file",
        type=str, default='', metavar='PATH',
        help="The file to copy ligatures from. If unspecified, ligaturize will"
        " attempt to pick a suitable one from fonts/fira/ based on the input"
        " font's weight."
    )
    parser.add_argument(
